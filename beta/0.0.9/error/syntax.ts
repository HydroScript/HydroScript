...
class HydroScriptSyntaxError extends Error {
    ...
    constructor(expected: null | TokenType | TokenType[], got: TokenType | string, line?: number, token?: Token) {
        let message: string
        if (expected == null)
        ...
        super(line ? `Line ${line}: ${message}${token ? `\nToken: ${JSON.stringify(token)}` : ""}` : message)
    }
}
