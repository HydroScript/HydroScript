...
export default class Compiler {
    ...
    compile(program: Program): string {
        ...
        for (let i = 0; i <= program.body.length - 1; i++) {
            ...
            result += this.compile_statement(statement, true)
        }
        if (result != "") result += "\n"
    ...
    compile_statement(statement: Statement, is_main: boolean, options?: Record<string, any>): string {
        ...
        switch (statement.kind) {
            ...
            case "ClassBlock":
                return this.compile_class_block(statement as ClassBlock, is_main)
            ...
            case "Identifier":
                return this.compile_identifier(statement as Identifier, is_main)
            case "StringLiteral":
            ...
            case "AssignmentExpression":
                return this.compile_assignment_expression(statement as AssignmentExpression, is_main)
            ...
            default:
                throw new Error(`statement kind ${statement.kind} not yet implemented in compiler`)
        }
    ...
    private compile_block(statement: Block, is_main: boolean): string {
        ...
        for (let i = 0; i <= statement.body.length - 1; i++) {
            ...
            result += this.compile_statement(current, true)
        }
        return `{${result}}${is_main ? ";" : ""}`
    }

    private compile_class_block(statement: ClassBlock, is_main: boolean): string {
        ...
        for (const stmt of definitions) definition_statements.push(this.compile_assignment_expression(stmt, true, false, false))
        const constructor_string: string = this.compile_function_literal(constructor, true, true)
        const initializer_expressions: string[] = []
        for (const init of initializers) initializer_expressions.push(this.compile_identifier(init, true))

        return `{${static_statements.join("")}${constructor_string}${initializer_expressions.join("")}${definition_statements.join("")}}`
    }
    ...
    private compile_if_statement(statement: IfStatement): string {
        ...
        const else_body = this.compile_block(statement.else, true)
        return `if (${condition}) ${body} else ${else_body}`
    }
    ...
    private compile_identifier(expression: Identifier, is_main: boolean): string {
        return `${add_suffix(expression.symbol)}${is_main ? ";" : ""}`
    }
    ...
    private compile_function_literal(expression: FunctionLiteral, is_main: boolean, is_constructor: boolean): string {
        ...
        const block = this.compile_block(body, false)

        if (is_constructor) return `constructor (${params.join(", ")}) ${block}${is_main ? ";" : ""}`
        return `(${asynchronous ? "async " : ""} function (${params.join(", ")}) ${block}${is_main ? ";" : ""})`
    }

    private compile_class_literal(expression: ClassLiteral, is_main: boolean): string {
        return `class${expression.extends ? ` extends ${this.compile_identifier(expression.extends, false)}` : ""} ${this.compile_class_block(expression.definition, is_main)}`
    }
    ...
    private compile_assignment_expression(expression: AssignmentExpression, is_main: boolean, init_var: boolean = true, brackets: boolean = true): string {
        if (expression.left.kind != "Identifier"
        && expression.left.kind != "MemberCallExpression") throw new Error(`left side of assignment expression is not identifier, got: ${expression.left.kind}`)
        if (init_var && expression.left.kind == "Identifier") this.variables.push(this.compile_identifier(expression.left as Identifier, false))
        const final = `${this.compile_statement(expression.left, false)} ${expression.operator} ${this.compile_statement(expression.right, false)}`
    ...
    private compile_function_call_expression(expression: FunctionCallExpression, is_main: boolean): string {
        const callee = this.compile_statement(expression.callee, false)
        let args: string[] = []
        for (let i = 0; i <= expression.arguments.length - 1; i++) {
            if (expression.arguments[i].object == "Token") throw new Error("unexpected token in function call expression")
            args.push(this.compile_statement(expression.arguments[i] as Expression, false))
        }
        return `(${callee}(${args.join(", ")}))${is_main ? ";" : ""}`
    }
