...
import ExistentialStatement from "../ast/existential_statement"
...
export default class Parser {
    ...
    public parse_expression(): Expression {
        if (this.at().type == TokenType.ArithmeticAssignment) return this.parse_arithmetic_assignment_operation(true)
            ...
            if (token_type == TokenType.Identifier || token_type == TokenType.Number) {
                ...
                switch (token_type) {
                    case TokenType.Identifier: {
                        ...
                        if (this.at().type == TokenType.ArithmeticAssignment) return this.parse_arithmetic_assignment_operation(false, id)
                        return id
                    }
        ...
        while (expression.object == "Expression") {
            ...
            if (next_token) {
                if (next_token.type == TokenType.OpenParenthesis) expression = this.parse_function_call_expression(expression as Expression)
                    ...
                    if (this.at().type == TokenType.ArithmeticAssignment) expression = this.parse_arithmetic_assignment_operation(false, expression as MemberCallExpression)                        
                }
                else break
            }
            else break
    ...
    private parse_arithmetic_assignment_operation(pre: boolean, id?: Identifier | MemberCallExpression): Expression {
    ...
    private parse_statement(options: Record<string, any> = {}): Statement {
        ...
        while (true) {
            if (this.at().type == TokenType.IfExistentialOperator || this.at().type == TokenType.UnlessExistentialOperator) {
                result = this.parse_existential_statement(result)
            } else break
        }
    ...
    private parse_existential_statement(statement: Statement): ExistentialStatement {

        const unless = this.eat().type == TokenType.UnlessExistentialOperator
        return {
            object: "Statement",
            kind: "ExistentialStatement",
            statement,
            condition: !unless ? this.find_expression() : {
                object: "Expression",
                kind: "LogicalNotExpression",
                target: this.find_expression()
            } as LogicalNotExpression
        } as ExistentialStatement
