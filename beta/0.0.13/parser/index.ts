...
export default class Parser {
    ...
    private expect(type: TokenType): Token {
        ...
        if (!previous || previous.type != type) throw_error(new HydroScriptSyntaxError(type, previous.type, this.path, previous.line, previous.column, previous))
        return previous
    }

    private expect_multiple(types: TokenType[]): Token {
        const previous = this.eat()
        if (!previous || !types.includes(previous.type)) throw_error(new HydroScriptSyntaxError(types, previous.type, this.path, previous.line, previous.column, previous))
        return previous
    }
    ...
    private parse_primary_expression(): Expression {
        ...
        let expression = ((): Expression => {
            ...
            throw_error(new HydroScriptSyntaxError(null, token_type, this.path, current_token.line, current_token.column, current_token))
