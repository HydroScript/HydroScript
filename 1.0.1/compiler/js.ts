...
export default class JavaScriptCompiler {
    ...
    private compile_statement(statement: Statement, is_main: boolean, options?: Record<string, any>): string {
    ...
    private compile_continue_statement(statement: ContinueStatement): string {
        return "continue;"
    }

    private compile_break_statement(statement: BreakStatement): string {
        return "break;"
    }
    ...
    private compile_if_statement(statement: IfStatement): string {
        ...
        return `if ${condition.startsWith("(") ? condition : `(${condition})`} ${body}${statement.else ? ` else ${else_body}` : ""}`
    }

    private compile_short_if_statement(statement: ShortIfStatement): string {
        const stmt = this.compile_statement(statement.statement, true)
        const condition = this.compile_statement(statement.condition, false)
        let else_body: string = ""
        if (statement.else) else_body = ` else ${this.compile_statement(statement.else, true)}`
        return `if ${condition.startsWith("(") ? condition : `(${condition})`} ${stmt}${else_body}`
    }

    private compile_loop_statement(statement: LoopStatement): string {

        switch (statement.loop.type) {
            case "While": {
                const condition = this.compile_statement((statement.loop as WhileLoop).condition, false)
                const body = this.compile_block((statement.loop as WhileLoop).body, true)
                return `while ${condition.startsWith("(") ? condition : `(${condition})`} ${body}`
            }

            case "For": {
                const initialize = this.compile_statement((statement.loop as ForLoop).initialize, true)
                const condition = this.compile_statement((statement.loop as ForLoop).condition, true)
                const afterthought = this.compile_statement((statement.loop as ForLoop).afterthought, false)
                const body = this.compile_block((statement.loop as ForLoop).body, true)
                return `for (${initialize} ${condition} ${afterthought}) ${body}`
            }

            case "ForIn": {
                const in_expr = this.compile_in_expression((statement.loop as ForInLoop).in, false, true)
                const body = this.compile_block((statement.loop as ForInLoop).body, true)
                return `for ${in_expr} ${body}`
            }

            case "ForOf": {
                const of_expr = this.compile_of_expression((statement.loop as ForOfLoop).of, false, true)
                const body = this.compile_block((statement.loop as ForOfLoop).body, false)
                return `for ${of_expr} ${body}`
            }
    ...
    private compile_try_statement(statement: TryStatement): string {
        ...
        return `${try_body}${catch_body}${finally_body};`
    }
    ...
    private compile_regular_expression(expression: RegularExpression, is_main: boolean): string {
        return `${expression.value}${is_main ? ";" : ""}`
    }
    ...
    private compile_in_expression(expression: InExpression, is_main: boolean, in_for: boolean = false): string {
        return `(${in_for ? "var " : ""}${this.compile_statement(expression.left, false)} in ${this.compile_statement(expression.right, false)})${is_main ? ";" : ""}`
    }

    private compile_of_expression(expression: OfExpression, is_main: boolean, in_for: boolean = false): string {
        return `(${in_for ? "var " : ""}${this.compile_statement(expression.left, false)} of ${this.compile_statement(expression.right, false)})${is_main ? ";" : ""}`
    }

    private compile_to_expression(expression: ToExpression, is_main: boolean): string {
        this.states.libs.__to = true
        const start = this.compile_statement(expression.start, false)
        const end = this.compile_statement(expression.end, false)
        const inc = this.compile_statement(expression.increment, false)
        return `__to(${start}, ${end}, ${inc})${is_main ? ";" : ""}`
    }
    ...
    private compile_void_expression(expression: VoidExpression, is_main: boolean): string {
        return `(void ${this.compile_statement(expression.value, false)})${is_main ? ";" : ""}`
    }
