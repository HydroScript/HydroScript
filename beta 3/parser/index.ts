...
export default class Parser {
    ...
    private expect(type: TokenType): Token {
        ...
        if (!previous || previous.type != type) throw new HydroScriptSyntaxError(type, previous.type)
        return previous
    }

    private expect_multiple(types: TokenType[]): Token {
        const previous = this.eat()
        if (!previous || !types.includes(previous.type)) throw new HydroScriptSyntaxError(types, previous.type)
        return previous
    }
    ...
    private parse_primary_expression(): Expression {
        ...
        let expression = ((): Expression => {
            ...
            throw new HydroScriptSyntaxError(null, token_type)
