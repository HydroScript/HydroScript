...
import { HydroScriptInternalError, HydroScriptParsingError, HydroScriptSyntaxError, HydroScriptCompileError, format_parsing_error } from "../error"
...
export default class Compiler {
    ...
    compile(program: Program): string {
        ...
        for (let i = 0; i <= program.body.length - 1; i++) {
            ...
            result += this.compile_statement(statement, true, { block_root: true })
        }
        if (result != "") result += "\n"
    ...
    compile_statement(statement: Statement, is_main: boolean, options?: Record<string, any>): string {
        ...
        switch (statement.kind) {
            ...
            case "ExportStatement":
                return this.compile_export_statement(statement as ExportStatement)
    ...
    private compile_export_statement(statement: ExportStatement): string {
        if (this.export) throw new HydroScriptCompileError("There cannot be more than one export statement")
    ...
    private compile_identifier(expression: Identifier, is_main: boolean, suffix: boolean = true, block_root: boolean = false): string {
        if (block_root) this.variables.push(add_suffix(expression.symbol))
    ...
    private compile_function_literal(expression: FunctionLiteral, is_main: boolean, is_constructor: boolean, method_name?: string): string {
        ...
        const block = (force_function != 1 && body.body.length == 1 && body.body[0].object == "Expression") ? block_string.substring(1, block_string.length - 2) : block_string
        ...
        if (force_function == 1 || !(force_function == 0 && body.body.length == 1 && body.body[0].object == "Expression")) return `(${asynchronous ? "async " : ""}function (${params.join(", ")}) ${block})${is_main ? ";" : ""}`
    ...
    private compile_function_call_expression(expression: FunctionCallExpression, is_main: boolean, parenthesis: boolean = true): string {
        ...
        return `${parenthesis ? "(" : ""}${callee}(${args.join(", ")})${parenthesis ? ")" : ""}${is_main ? ";" : ""}`
    }
