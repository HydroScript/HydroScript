...
export default class Parser {
    ...
    private parse_comparison_expression(): Expression {
        ...
        let left = this.parse_of_expression()

        while (this.at().type == TokenType.ComparisonOperator) {

            const operator = this.eat().value
            const right = this.parse_of_expression()
            left = {
                object: "Expression",
    ...
    private parse_of_expression(): Expression {

        let left = this.parse_in_expression()

        while (this.at().type == TokenType.Of) {

            const operator = this.eat().value
            const right = this.parse_in_expression()
            left = {
                object: "Expression",
                kind: "OfExpression",
                left,
    ...
    private parse_in_expression(): Expression {
    ...
    private parse_primary_expression(): Expression {
        ...
        let expression = ((): Expression => {
            ...
            } else if (token_type == TokenType.Local) {

                return this.parse_local_assignment_expression()
        ...
        while (expression.object == "Expression") {
            ...
            if (next_token) {
                ...
                } else if (this.at().type == TokenType.To && (
    ...
    private parse_local_assignment_expression(): LocalAssignmentExpression {

        const op = this.expect(TokenType.Local)
        const expr = this.find_expression()
        if (expr.kind != "AssignmentExpression") throw new HydroScriptParsingError(format_parsing_error("AssignmentExpression", expr.kind))
        return {
            object: "Expression",
            kind: "LocalAssignmentExpression",
            assignment: expr as AssignmentExpression
        } as LocalAssignmentExpression

    }

    private parse_unary_expression(): UnaryExpression {
    ...
    private parse_to_expression(expr: Expression): ToExpression {
        
        const mark = this.expect(TokenType.To)
        const end = this.parse_primary_expression()
        if (end.kind != "NumericLiteral" && end.kind != "Identifier" && end.kind != "MemberCallExpression")
            throw new HydroScriptParsingError(format_parsing_error(
                [ "NumericLiteral", "Identifier", "MemberCallExpression" ],
                end.kind
            ))
        let inc: NumericLiteral | Identifier | MemberCallExpression = {
            object: "Expression",
            kind: "NumericLiteral",
            value: "1"
        } as NumericLiteral
        if (this.at().type == TokenType.Increment && this.eat()) {
            const incr = this.parse_primary_expression()
            ...
            switch (incr.kind) {
                case "NumericLiteral": {
                    inc = incr as NumericLiteral
                    break
                }
                case "Identifier": {
                    inc = incr as Identifier
                    break
                }
                ...
                default:
                    throw new HydroScriptInternalError("Invalid expression kind")
            }
        }
        return {
            object: "Expression",
    ...
    private parse_call_expression(expression: Expression): FunctionCallExpression {

        const op = this.expect(TokenType.Call)
        const func = this.find_expression()
        return {
            object: "Expression",
    ...
    public parse_statement(options: Record<string, any> = {}): Statement {
        ...
        let result = ((): Statement => {
            ...
            if (this.at().type == TokenType.Declarator) return this.parse_declaration_statement()
            if (this.at().type == TokenType.Continue) return this.parse_continue_statement()
            if (this.at().type == TokenType.Break) return this.parse_break_statement()
            ...
            while (result.object == "Expression") {
                if (this.at().type == TokenType.QuestionMark) {
                    ...
                    continue
                } else if (this.at().type == TokenType.Loop) {
                    result = this.parse_loop_statement(result as Expression)
                    break
                } else if (this.at().type == TokenType.Comma) {
                    result = this.parse_comma_expression(result as Expression)
                    continue
                } else if (this.at().type == TokenType.Call) {
                    result = this.parse_call_expression(result as Expression)
                    continue
                } else break
            }
    ...
    private parse_class_block(): ClassBlock {
        ...
        while (true) {
            ...
            const statement = current.type == TokenType.Get ?
                this.parse_getter() : current.type == TokenType.Set ?
                this.parse_setter() : this.parse_statement({ in_class: true })
            if (statement.object == "Statement" && statement.kind != "StaticPropertyDeclaration" && statement.kind != "Getter" && statement.kind != "Setter") throw new HydroScriptParsingError(format_parsing_error([ "Expression", "StaticPropertyDeclaration" ], statement.kind))
            statements.push(statement)
        }
        const end = this.expect(TokenType.CloseCurlyBracket)
        ...
        for (let i = 0; i <= statements.length - 1; i++) {
            ...
            else if (current.kind == "Getter") getters.push(current as Getter)
            else if (current.kind == "Setter") setters.push(current as Setter)
    ...
    private parse_getter(): Getter {

        const op = this.expect(TokenType.Get)
    ...
    private parse_setter(): Setter {

        const op = this.expect(TokenType.Set)
        const name = this.parse_identifier()
        const parameter = this.parse_identifier()
    ...
    private parse_declaration_statement(): DeclarationStatement {

        const op = this.expect(TokenType.Declarator)
        const target = this.parse_identifier()
        const value = this.find_expression()
        return {
            object: "Statement",
    ...
    private parse_continue_statement(): ContinueStatement {
        ...
        const op = this.expect(TokenType.Continue)
        return {
            object: "Statement",
    ...
    private parse_break_statement(): BreakStatement {
        ...
        const op = this.expect(TokenType.Break)
        return {
            object: "Statement",
            kind: "BreakStatement"
        } as BreakStatement
    ...
    private parse_static_property_declaration(in_class: boolean): StaticPropertyDeclaration {
        ...
        return {
            ...
            expression: this.at().type == TokenType.Get ?
                this.parse_getter() : this.at().type == TokenType.Set ?
                this.parse_setter() : {
                    object: "Expression",
                    kind: "AssignmentExpression",
    ...
    private parse_loop_statement(expression: Expression): LoopStatement {

        const mark = this.expect(TokenType.Loop)
        const loop = (expr => {

            switch (expr.kind) {
                case "CommaExpression": {

                    if ((expr as CommaExpression).expressions.length == 3) {
                        const [ initialize, condition, afterthought ] = (expr as CommaExpression).expressions
                        return {
                            type: "For",
                            initialize,
                            condition,
                ...
                case "InExpression":
                    if ((expr as InExpression).left.kind != "Identifier")
                        throw new HydroScriptParsingError(format_parsing_error("Identifier", (expr as InExpression).left.kind))
                    return {
                        type: "ForIn",
                        in: expr as InExpression,
                        body: this.parse_block()
                    } as ForInLoop
                case "OfExpression":
                    if ((expr as OfExpression).left.kind != "Identifier")
                        throw new HydroScriptParsingError(format_parsing_error("Identifier", (expr as InExpression).left.kind))
                    return {
                        type: "ForOf",
                        of: expr as OfExpression,
                        body: this.parse_block()
                    } as ForOfLoop
                default:
                    return {
    ...
    private parse_function_parameters(): Identifier[] {
        ...
        while (
            this.at().type == TokenType.Identifier && (
                this.next().type != TokenType.BinaryOperator &&
                this.next().type != TokenType.ComparisonOperator &&
                this.next().type != TokenType.OpenParenthesis &&
                this.next().type != TokenType.CloseParenthesis &&
                this.next().type != TokenType.QuestionMark &&
                this.next().type != TokenType.ShortIfOperator &&
                this.next().type != TokenType.Unless &&
                this.next().type != TokenType.ShortUnlessOperator &&
                this.next().type != TokenType.Loop &&
                this.next().type != TokenType.AssignmentOperator &&
                this.next().type != TokenType.LogicalOperator &&
                this.next().type != TokenType.BitwiseOperator &&
                this.next().type != TokenType.InstanceOf &&
                this.next().type != TokenType.In &&
                this.next().type != TokenType.Of &&
                this.next().type != TokenType.To &&
                this.next().type != TokenType.Dot &&
                this.next().type != TokenType.Comma &&
                this.next().type != TokenType.FileEnd
            )
