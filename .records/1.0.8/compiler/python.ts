...
export default class PythonCompiler {
    ...
    private compile_statement(statement: Statement, indents: number = 0): string {
        ...
        switch (statement.kind) {
            ...
            case "FunctionCallExpression":
                return this.compile_function_call_expression(statement as FunctionCallExpression, indents)
    ...
    private compile_function_call_expression(expression: FunctionCallExpression, indents: number): string {
        const callee = this.compile_statement(expression.callee, indents)
        const args = expression.arguments.expressions.map(v => this.compile_statement(v)).join(", ")
        return `${callee}(${args})`
    }
