...
class HydroScriptSyntaxError extends Error {
    ...
    constructor(expected: null | TokenType | TokenType[], got: TokenType | string, path: string, line?: number, column?: number, process_type?: ProcessType) {
        ...
        if (line) {
            this._line = line
            this._column = column
            stack.push(`    at <unknown> (${this._path}:${line}:${column})`)
        }
        if (typeof process_type == "number") {
            const lex_stack = `    at HydroScript.Tokenize (hydroscript\\lexer\\index.js)`
            const parse_stack = `    at HydroScript.Parse (hydroscript\\parser\\index.js)`
            const compile_stack = `    at HydroScript.Compile (hydroscript\\compiler\\index.js)`
            switch (process_type) {
        ...
        this._display = ((): string => {
            ...
            const start = Math.max(0, this._column - 32)
            const end = Math.min(error_line.length, this._column + 32)
